...
def add_heats(snapshot, duration, pixels, time_unit, space_unit, addr_range):
    ...
    for region in snapshot.regions:
        ...
        while fraction_start < end:
            ...
            heat = (
                region.nr_accesses.samples * duration * (fraction_end - fraction_start)
            )
...
def heat_pixels_from_snapshots(snapshots, time_range, addr_range, resols):
    ...
    pixels = [
        [
            HeatPixel(
                int(time_range[0] + i * time_unit),
                int(addr_range[0] + j * space_unit),
                0.0,
            )
...
def heatmap_plot_ascii(pixels, time_range, addr_range, resols, colorset):
    ...
    colorsets = {
        "gray": [[232] * 10, [237, 239, 241, 243, 245, 247, 249, 251, 253, 255]],
        "flame": [
    ...
    for snapshot in pixels:
        ...
        for pixel in snapshot:
            ...
            bg = colors[0][heat]
            fg = colors[1][heat]
            chars.append("\u001b[48;5;%dm\u001b[38;5;%dm%d" % (bg, fg, heat))
        print("".join(chars) + "\u001b[0m")
    color_samples = [
        "\u001b[48;5;%dm\u001b[38;5;%dm %d " % (colors[0][i], colors[1][i], i)
        for i in range(10)
    ]
    print("# access_frequency: %s" % "".join(color_samples) + "\u001b[0m")
    print(
        "# x-axis: space (%d-%d: %s)"
        % (
            addr_range[0],
    ...
    print(
        "# resolution: %dx%d (%s and %s for each character)"
        % (
            len(pixels[1]),
            len(pixels),
...
def pr_heats(args, damon_result):
    ...
    pixels = heat_pixels_from_snapshots(
        snapshots, [tmin, tmax], [amin, amax], [tres, ares]
    )

    if args.heatmap == "stdout":
        heatmap_plot_ascii(
            pixels, [tmin, tmax], [amin, amax], [tres, ares], args.stdout_heatmap_color
        )
        return
    ...
    def __str__(self):
        ...
        lines.append(
            ...
            % (
                self.start_time,
                self.end_time,
                _damo_fmt_str.format_time_ns(self.end_time - self.start_time, False),
            )
        )
...
def get_guide_info(damon_result):
    ...
    return sorted(list(guides.values()), key=lambda x: x.total_space(), reverse=True)
...
def set_missed_args(args, damon_result):
    ...
    guide = guides[0]
    # print_functions(guide)
    # print(guide.total_space())
    # print(guide.regions())
    ...
    if not args.address_range:
        # if not args.heap:
        #     args.address_range = sorted(
        #         guide.regions(), key=lambda x: x[1] - x[0], reverse=True
        #     )[0]
        # else:
        args.address_range = addr_ranges[0]
        # print("using range {}-{}".format(args.address_range[0], args.address_range[1]))
...
def plot_heatmap(data_file, output_file, args):
    ...
    plot '%s' using ($1 / 1000000000):($2 / (1024*1024)):3 with image;""" % (
        terminal,
        output_file,
        x_range[0],
        x_range[1],
...
def set_argparser(parser):
    ...
    parser.add_argument(
        "--heap",
        metavar="store_true",
        help="weather to identify heap or mmaped region",
    )
    parser.add_argument(
        "--stdout_heatmap_color",
...
def main(args=None):
    ...
    if err != None:
        print("monitoring result file (%s) parsing failed (%s)" % (args.input, err))
        exit(1)
    ...
    if args.guide:
        ...
        print("finished getting all ranges")
        # end = time.time()
        # print("time for set_missed_args:", end - start)
